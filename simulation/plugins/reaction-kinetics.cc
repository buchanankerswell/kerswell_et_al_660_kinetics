...
namespace aspect
{
  namespace MaterialModel
  {
          ...
          if (in.requests_property(MaterialProperties::viscosity))
            {
              ...
              if (reaction_rate_out != nullptr)
                reaction_kinetics_out->visc_temperature_dependence[q] = visc_temperature_dependence;
            }
          ...
          if (reaction_rate_out != nullptr)
            {
              ...
              auto clamp_exp = [](double x) { return std::exp(std::max(-700.0, std::min(700.0, x))); };
              ...
              auto compute_rate = [&](double gibbs, double saturation, bool write_thermo) -> double {
                ...
                if (write_thermo)
                  reaction_kinetics_out->thermodynamic[q] = thermodynamic_driving_force;
    ...
    template <int dim>
    void
    ReactionKinetics<dim>::create_additional_named_outputs(MaterialModel::MaterialModelOutputs<dim> &out) const
    {
